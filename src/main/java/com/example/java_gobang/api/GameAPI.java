...
public class GameAPI extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        //  因此前面匹配到对手之后,需要经过页面跳转,来到 game_hall.html 才算正式进入游戏房间(才算玩家准备就绪)
        //  当前这个逻辑是在 game_room.html 页面加载的时候进行的
        //  执行到当前逻辑,说明玩家已经页面跳转成功了!!
        //  页面跳转,其实是个大活~ (很 有可能出现"失败"的情况的)
        synchronized (room){
            ...
            if(room.getUser2() == null){
                // 如果进入到这个逻辑,说明玩家1 已经加入房间现在要给当前玩家作为玩家2 了
                room.setUser2(user);
                System.out.println("玩家 "+user.getUsername()+" 已经准备就绪! 作为玩家2");

                // 当两个玩家都加入了成功之后,就要让服务器,给这两个玩家都返回 websocket 的响应数据.
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        ...
        if(session == exitSession) {
            // 加上这个判定,目的是为了避免再多开的情况下,第二个用户退出连接动作,导致第一个用户的会话被删除.
