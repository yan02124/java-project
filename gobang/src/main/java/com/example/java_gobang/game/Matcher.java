...
public class Matcher {
    ...
    public void remove(User user) {
        if(user.getScore() < 2000){
            ...
            System.out.println("把玩家 "+user.getUsername() + "移除到了 normalQueue 中");
        }else if(user.getScore() >= 2000 && user.getScore() < 3000){
            synchronized (highQueue){
                highQueue.remove(user);
            }
            System.out.println("把玩家 "+user.getUsername() + "移除到了 highQueue 中");
        }else {
            synchronized (veryHighQueue){
                veryHighQueue.remove(user);
            }
            System.out.println("把玩家 "+user.getUsername() + "移除到了 veryHighQueue 中");
        }
    }
    ...
    private void handlerMatch(Queue<User> matchQueue) {
        synchronized (matchQueue){
            try{
                ...
                //此时当前能否排到两个玩家是同一个用户的情况嘛?一个玩家入队列了两次？？
                //理论上也不会存在！
